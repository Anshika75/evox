...
import { blogSectionContent, blogPosts as originalBlogPosts } from "@/data/blog-data"
...
export default function BlogSection() {
  ...
  const extendedBlogPosts = [
    ...originalBlogPosts.slice(-clonesAtEachEnd),
    ...originalBlogPosts,
    ...originalBlogPosts.slice(0, clonesAtEachEnd),
  ]
  ...
  const totalRealSlides = Math.ceil(originalBlogPosts.length / visibleCardsCount)
  ...
  const handleTransitionEnd = useCallback(() => {
    ...
    if (currentSlide >= originalBlogPosts.length + clonesAtEachEnd) {
      goToSlide(clonesAtEachEnd, false)
    } else if (currentSlide < clonesAtEachEnd) {
      goToSlide(originalBlogPosts.length + clonesAtEachEnd - 1, false)
    }
  }, [currentSlide, clonesAtEachEnd, originalBlogPosts.length, goToSlide])
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-12">
        ...
        <div className="flex justify-between items-center sm:items-start mb-12 lg:mb-20">
          ...
          {totalRealSlides > 1 && (
            <div className="flex space-x-4 pt-2">
              {" "}
              {/* Added pt-2 to align with "our blog" */}
              <button
                onClick={prevSlide}
          ...
          <div className="text-right">
            <p className="text-[#331249] text-sm sm:text-3xl tracking-[7px] plus-jakarta-sans-medium font-medium">{blogSectionContent.preHeading}</p>
            <h2 className="text-3xl lg:text-5xl font-bold plus-jakarta-sans-bold text-gray-900 leading-tight">{blogSectionContent.heading}</h2>
          </div>
        </div>
        ...
        <div className="relative">
          <div className="overflow-hidden">
            ...
            >
              {extendedBlogPosts.map((post: BlogPost, index: number) => (
  ...
  return (
    <div className="bg-[#F8F8F6] transition-all duration-500 cursor-pointer hover:shadow-lg overflow-hidden">
      <div className="relative h-100 w-full">
        <Image src={post.image || "/placeholder.svg"} height={500} width={500} alt={post.alt} className="w-full h-full object-cover" />
      ...
      <div className="p-6 mt-4">
        ...
        <h3 className="text-2xl font-semibold plus-jakarta-sans-semibold text-custom-black mb-4 leading-tight">{post.title}</h3>
        <Link
          href={post.link}
